...
export default function QuizStudentResultPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await quizAPI.getStudentResult(quizId, studentId);
  ...
  const submitGrading = async (answerId: number, maxPoints: number) => {
    ...
    try {
      await quizAPI.gradeAnswer(quizId, answerId, {
        score,
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2">Siswa belum mengerjakan quiz ini.</p>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">{result.student.name} - NIS: {result.student.nisn}</p>
          </div>
        </div>
              ...
              <div key={answer.id} className="p-4">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 flex flex-col items-center gap-1">
                    <span className="w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-full font-bold text-slate-700 dark:text-slate-300">
                      {index + 1}
                    </span>
