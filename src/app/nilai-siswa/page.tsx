  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Nilai Saya</h1>
          <p className="text-slate-600 dark:text-slate-400">Lihat rekap nilai ujian dan tugas Anda</p>
        </div>
        ...
        {(activeTab === 'ujian' || activeTab === 'gabungan') && (
          <Card className="overflow-hidden">
            <div className="p-4 border-b flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Nilai Ujian</h2>
              <span className="text-sm text-slate-600 dark:text-slate-400">({examGrades.length})</span>
            </div>

            {examGrades.length === 0 ? (
              <div className="p-8 text-center">
                <FileText className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-4" />
                <p className="text-slate-600 dark:text-slate-400">Belum ada riwayat nilai ujian</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50 dark:bg-slate-800">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Ujian</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Mata Pelajaran</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Benar/Salah</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Nilai</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Status</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Tanggal</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200">
                    {examGrades.map((grade) => (
                      <tr key={`exam-${grade.id}`} className="hover:bg-slate-50 dark:hover:bg-slate-800">
        ...
        {(activeTab === 'tugas' || activeTab === 'gabungan') && (
          <Card className="overflow-hidden">
            <div className="p-4 border-b flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Nilai Tugas</h2>
              <span className="text-sm text-slate-600 dark:text-slate-400">({assignmentGrades.length})</span>
            </div>

            {assignmentGrades.length === 0 ? (
              <div className="p-8 text-center">
                <FileText className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-4" />
                <p className="text-slate-600 dark:text-slate-400">Belum ada riwayat nilai tugas</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50 dark:bg-slate-800">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Tugas</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Mata Pelajaran</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Skor</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Nilai</th>
                      <th className="px-6 py-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Status</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 dark:text-slate-400 uppercase">Tanggal</th>
                    </tr>
                  ...
                  <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200">
                    {assignmentGrades.map((grade) => (
                      <tr key={`assignment-${grade.id}`} className="hover:bg-slate-50 dark:hover:bg-slate-800">
                        <td className="px-6 py-4 whitespace-nowrap">
