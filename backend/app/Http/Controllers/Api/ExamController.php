...
class ExamController extends Controller
{
    ...
    public function republishHistory(Request $request, Exam $exam)
    {
        ...
        $historySourceExamId = (int) $exam->id;

        $archives = ExamRepublishArchive::with('republisher:id,name')
            ->where('exam_id', $historySourceExamId)
            ->orderByDesc('session_no')
            ->get();

        // Jika request datang dari ujian clone, arsip ada di exam sumber.
        if ($archives->isEmpty()) {
            ...
            if ($sourceArchive) {
                $historySourceExamId = (int) $sourceArchive->exam_id;
                $archives = ExamRepublishArchive::with('republisher:id,name')
                    ->where('exam_id', $historySourceExamId)
                    ->orderByDesc('session_no')
                    ->get();
        ...
        return response()->json([
            ...
            'data' => [
                'exam_id' => $exam->id,
                'source_exam_id' => $historySourceExamId,
    ...
    /**
     * Restore result/answer snapshot from republish history.
     */
    public function restoreRepublishHistory(Request $request, Exam $exam, int $sessionNo)
    {
        $user = $request->user();
        ...
        if ($user->role !== 'admin') {
            return response()->json([
                'success' => false,
                'message' => 'Hanya admin yang dapat melakukan restore arsip re-publish',
            ], 403);
        }

        $validated = $request->validate([
            'target_exam_id' => 'nullable|integer|exists:exams,id',
        ]);

        $targetExamId = (int) ($validated['target_exam_id'] ?? $exam->id);
        $archive = ExamRepublishArchive::where('exam_id', $exam->id)
        ...
        if (!$archive) {
            return response()->json([
                'success' => false,
                'message' => "Arsip tidak ditemukan untuk exam_id={$exam->id}, session_no={$sessionNo}",
            ], 404);
        }
        ...
        if (!$targetExam) {
            return response()->json([
                'success' => false,
                'message' => "Ujian target tidak ditemukan: {$targetExamId}",
            ], 404);
        }
        ...
        if (empty($resultRows) && empty($answerRows)) {
            return response()->json([
                'success' => false,
                'message' => 'Arsip ditemukan, tetapi tidak ada result_rows/answer_rows untuk direstore.',
            ], 422);
        }
        ...
        $this->forgetExamShowCache($targetExamId);

        try {
            AuditLog::create([
                'user_id' => $user->id,
                'action' => 'exam.republish.restore',
                'description' => "Restore arsip re-publish exam {$exam->id} sesi {$sessionNo} ke exam {$targetExamId}",
                'target_type' => 'Exam',
                'target_id' => $targetExamId,
                'ip_address' => $request->ip(),
                'user_agent' => $request->userAgent(),
                'new_values' => [
                    'source_exam_id' => $exam->id,
                    ...
                    'result_restored' => $restoredResults,
                    'answer_restored' => $restoredAnswers,
                    'answer_skipped' => $skippedAnswers,
                ],
            ]);
        } catch (\Throwable $e) {
            Log::warning('Audit log restore republish archive failed: ' . $e->getMessage());
        }

        return response()->json([
            'success' => true,
            'message' => 'Restore arsip re-publish berhasil dijalankan.',
            'data' => [
                'source_exam_id' => $exam->id,
                'session_no' => $sessionNo,
                'target_exam_id' => $targetExamId,
